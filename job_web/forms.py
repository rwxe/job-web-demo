...
from .models import Admin, User, Company, db, Job, FINANCE_STAGE, FIELD, EDUCATION, EXP
...
class RegisterUserForm(FlaskForm):
    ...
    def validate_email(self, field):
        if User.query.filter_by(email=field.data).first() or \
                Company.query.filter_by(email=field.data).first() or \
                Admin.query.filter_by(email=field.data).first():
...
class RegisterAdminForm(RegisterUserForm):
    ...
    def create_admin(self):
        admin = Admin()
        admin.name = self.name.data
        admin.email = self.email.data
        admin.password = self.password.data
        admin.id = int(time.time() * 100000000)
...
class RegisterCompanyForm(FlaskForm):
    ...
    finance_stage = SelectField(
        '融资阶段', choices=[(i, i) for i in FINANCE_STAGE])
    field = SelectField('行业领域', choices=[(i, i) for i in FIELD])
    description = StringField(
        '公司简介', validators=[Length(0, 50, message='最多50个字符')])
    submit = SubmitField('提交')
...
class CompanyDetailForm(FlaskForm):
    ...
    address = StringField('办公地址', validators=[
    ...
    description = TextAreaField(
        '企业简介', validators=[Length(0, 50, message='最多50个字符')])
    details = TextAreaField(
        '企业详情', validators=[Length(0, 1000, message='最多1000个字符')])
    submit = SubmitField('提交')
...
class JobForm(FlaskForm):
    ...
    name = StringField('职位名称', validators=[
                       DataRequired(message='请填写内容'), Length(4, 64)])
    salary_min = IntegerField('最低薪水（单位：千元）', validators=[
                              DataRequired(message='请填写整数')])
    salary_max = IntegerField('最高薪水（单位：千元）', validators=[
                              DataRequired(message='请填写整数')])
    ...
    treatment = TextAreaField(
        '职位待遇', validators=[Length(0, 256, message='最多256个字符')])
    description = TextAreaField(
        '职位描述', validators=[DataRequired(message='请填写内容')])
    is_enable = SelectField(
        '发布', choices=[(True, '立即发布'), (False, '暂不发布')], coerce=bool)
    submit = SubmitField('提交')
